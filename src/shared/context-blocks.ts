 ...
 *
 * Two things ride along: what the user wrote about themselves (global context), and who
 * they are from the directory (quick context). This assembles them in a stable order,
 * dropping any that are empty, so an empty About-you file adds nothing and the prompt does
 * not grow a blank heading.
 *
...
// facts. Empty blocks are dropped so nothing frames a void.
export const buildContextBlocks = (input: ContextBlockInput): readonly string[] => [aboutYouBlock(input.aboutYou), input.quickContext].filter((block) => block.trim().length > 0);
