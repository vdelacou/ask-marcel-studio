...
describe('the always-on context the agent gets', () => {
  test('who the user says they are rides along, under its own heading', () => {
    const blocks = buildContextBlocks({ aboutYou: 'I am the CIO. I care about clarity.', quickContext: '' });
  ...
  test('an empty About-you file adds no block, and no blank heading', () => {
    expect(buildContextBlocks({ aboutYou: '   ', quickContext: 'quick' })).toEqual(['quick']);
  });

  test('the order is who-they-are, then the directory', () => {
    const blocks = buildContextBlocks({ aboutYou: 'me', quickContext: 'directory' });

    expect(blocks).toEqual(['## About the person you work for\n\nme', 'directory']);
  });

  test('everything empty is no blocks at all, not a list of blanks', () => {
    expect(buildContextBlocks({ aboutYou: '', quickContext: '' })).toEqual([]);
  });
});
