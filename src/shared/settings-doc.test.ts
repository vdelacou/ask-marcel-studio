...
describe('a settings file that carries a section this version no longer knows', () => {
  const openai = { id: 'google', kind: 'openai', label: 'Google', baseUrl: 'https://x/v1', apiKey: { enc: 'AAA' }, modelIds: ['e'] };

  test('the memory section an older version wrote is dropped, not refused', () => {
    const parsed = parseStoredSettings({ providers: [openai], memory: { providerId: 'google', embeddingModelId: 'e' } });
