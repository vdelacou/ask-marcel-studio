...
- PowerPoint / layout-critical: \`download-drive-item-as-pdf --drive-id … --item-id … --output-path deck.pdf\`, then Read the PDF. A PDF over ~10 pages needs Read's \`pages\` parameter (e.g. \`pages: "1-15"\`), at most 20 pages per call — read a longer deck in 20-page chunks; that page cap is the Read tool's, not the CLI's.
